...
/*
	Handler for window SW_WINDOW ("Scale (Slip, Width)")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
SW_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
SW_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CREATE:
		...
		*/
		...
		setScrollRange (xdWindow, SW_SLIP_BAR, -10000, 10000);
		setScrollRange (xdWindow, SW_WIDTH_BAR, 0, 20000);
		/* TAG END EVNT:Create */
		}
		break;
	case E_DESTROY:
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case SW_SLIP_BAR: /* "slip bar" */
			{
			/* TAG BEGIN SW_SLIP_BAR EVNT:Control */
			int pos = 0;
			/*
			...
			*/
			...
			updateNumericTextField (xdWindow, SW_SLIP, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END SW_SLIP_BAR EVNT:Control */
			}
			break;
		case SW_WIDTH_BAR: /* "width bar" */
			{
			/* TAG BEGIN SW_WIDTH_BAR EVNT:Control */
			int pos = 0;
			/*
				Horizontal scrollbar control was operated
			*/
			...
			pos = updateScrollField (xdWindow, xdControlId, pos);
			updateNumericTextField (xdWindow, SW_WIDTH, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END SW_WIDTH_BAR EVNT:Control */
			}
			break;
		case SW_SLIP: /* "slip" */
			{
			/* TAG BEGIN SW_SLIP EVNT:Control */
			/*
			...
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				*/
			updateScrollAndText (xdWindow, SW_SLIP_BAR, SW_SLIP, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END SW_SLIP EVNT:Control */
			}
			break;
		case SW_WIDTH: /* "width" */
			{
			/* TAG BEGIN SW_WIDTH EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				/*
					Contents of control were changed
				*/
			updateScrollAndText (xdWindow, SW_WIDTH_BAR, SW_WIDTH, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END SW_WIDTH EVNT:Control */
			}
			break;
		default:
			break;
