...
*/
#define WIN_RES_ID PARALLEL_OPTIONS_WINDOW
...
/*
	Handler for window PARALLEL_OPTIONS_WINDOW ("Remote Options")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
PARALLEL_OPTIONS_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
PARALLEL_OPTIONS_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CHAR:
		...
		*/
		...
		if (xdEvent->v.chr.ch == '\r')
		{
		   ...
		   xdEvent->v.ctl.id = PARALLEL_OPTIONS_WINDO_C1;
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case PARALLEL_OPTIONS_ADDRESS: /* "Medusa" */
			{
			/* TAG BEGIN PARALLEL_OPTIONS_ADDRESS EVNT:Control */
			/*
			...
			*/
			...
			/* TAG END PARALLEL_OPTIONS_ADDRESS EVNT:Control */
			}
			break;
		case PARALLEL_OPTIONS_LOGIN: /* "paul" */
			{
			/* TAG BEGIN PARALLEL_OPTIONS_LOGIN EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
			...
			/* TAG END PARALLEL_OPTIONS_LOGIN EVNT:Control */
			}
			break;
		case PARALLEL_OPTIONS_WINDO_C1: /* "OK" */
			{
			/* TAG BEGIN PARALLEL_OPTIONS_WINDO_C1 EVNT:Control */
			saveParallelOptions (xdWindow);
			xvt_vobj_destroy(xdWindow);
			/* TAG END PARALLEL_OPTIONS_WINDO_C1 EVNT:Control */
			}
			break;
		case PARALLEL_OPTIONS_WINDO_C2: /* "Cancel" */
			{
			/* TAG BEGIN PARALLEL_OPTIONS_WINDO_C2 EVNT:Control */
			WINDOW win = (WINDOW) xvt_vobj_get_data(xdWindow);
						/* make sure proj options not set to Parallel 
						** after CANCEL is pressed */
			xdCheckRadioButton(win, LOCAL_REMOTE_LOCAL, LOCAL_REMOTE_REMOTE, LOCAL_REMOTE_LOCAL);
			xvt_vobj_destroy(xdWindow);
			/* TAG END PARALLEL_OPTIONS_WINDO_C2 EVNT:Control */
			}
			break;
		case PARALLEL_OPTIONS_WINDO_C3: /* "Help..." */
			{
			/* TAG BEGIN PARALLEL_OPTIONS_WINDO_C3 EVNT:Control */
			displayHelp("toc.htm");
			/* TAG END PARALLEL_OPTIONS_WINDO_C3 EVNT:Control */
			}
			break;
		case PARALLEL_OPTIONS_PATH: /* "./parNoddy" */
			{
			/* TAG BEGIN PARALLEL_OPTIONS_PATH EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
			...
			/* TAG END PARALLEL_OPTIONS_PATH EVNT:Control */
			}
			break;
		default:
			break;
