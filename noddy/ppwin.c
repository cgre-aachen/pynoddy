...
/*
	Handler for window PP_WINDOW ("Ori (PP)")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
PP_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
PP_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CREATE:
		...
		*/
		...
		setScrollRange (xdWindow, PP_PDIR_BAR, 0, 360);
		setScrollRange (xdWindow, PP_PLUNGE_BAR, -90, 90);
		/* TAG END EVNT:Create */
		}
		break;
	case E_DESTROY:
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case PP_PDIR_BAR: /* "plunge dir bar" */
			{
			/* TAG BEGIN PP_PDIR_BAR EVNT:Control */
			int pos = 0;
			/*
			...
			*/
			...
			updateNumericTextField (xdWindow, PP_PDIR, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END PP_PDIR_BAR EVNT:Control */
			}
			break;
		case PP_PLUNGE_BAR: /* "plunge bar" */
			{
			/* TAG BEGIN PP_PLUNGE_BAR EVNT:Control */
			int pos = 0;
			/*
				Horizontal scrollbar control was operated
			*/
			...
			pos = updateScrollField (xdWindow, xdControlId, pos);
			updateNumericTextField (xdWindow, PP_PLUNGE, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END PP_PLUNGE_BAR EVNT:Control */
			}
			break;
		case PP_PDIR: /* "plunge dir" */
			{
			/* TAG BEGIN PP_PDIR EVNT:Control */
			/*
			...
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				*/
			updateScrollAndText (xdWindow, PP_PDIR_BAR, PP_PDIR, TRUE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END PP_PDIR EVNT:Control */
			}
			break;
		case PP_PLUNGE: /* "plunge" */
			{
			/* TAG BEGIN PP_PLUNGE EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				/*
					Contents of control were changed
				*/
			updateScrollAndText (xdWindow, PP_PLUNGE_BAR, PP_PLUNGE, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END PP_PLUNGE EVNT:Control */
			}
			break;
		default:
			break;
