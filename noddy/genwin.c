...
*/
#define WIN_RES_ID GENERATE_WINDOW
...
/*
	Handler for window GENERATE_WINDOW ("Generate")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
GENERATE_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
GENERATE_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case GENERATE_GRAVITY: /* "Gravity" */
			{
			/* TAG BEGIN GENERATE_GRAVITY EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, GENERATE_GRAVITY);
			xvt_ctl_set_checked(ctl_win, (BOOLEAN) (!xvt_ctl_is_checked(ctl_win)));
			/* TAG END GENERATE_GRAVITY EVNT:Control */
			}
			break;
		case GENERATE_MAGNETICS: /* "Magnetics" */
			{
			/* TAG BEGIN GENERATE_MAGNETICS EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, GENERATE_MAGNETICS);
			xvt_ctl_set_checked(ctl_win, (BOOLEAN) (!xvt_ctl_is_checked(ctl_win)));
			updateOptions (xdWindow, GEOPHYSICS_CALC_OP);
			/* TAG END GENERATE_MAGNETICS EVNT:Control */
			}
			break;
		case GENERATE_COMPONENTS: /* "XYZ Components" */
			{
			/* TAG BEGIN GENERATE_COMPONENTS EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, GENERATE_COMPONENTS);
			xvt_ctl_set_checked(ctl_win, (BOOLEAN) (!xvt_ctl_is_checked(ctl_win)));
			/* TAG END GENERATE_COMPONENTS EVNT:Control */
			}
			break;
		default:
			break;
