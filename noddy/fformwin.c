...
*/
#define WIN_RES_ID FFOLD_WINDOW
...
/*
	Handler for window FFOLD_WINDOW ("Form (fold)")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
FFOLD_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
FFOLD_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case FFORM_PROFILE: /* "Show Profile..." */
			{
			/* TAG BEGIN FFORM_PROFILE EVNT:Control */
			extern long profileEventHandler ();
			char option[100];
			WINDOW win;
			...
			break;
		case FFORM_TYPE: /* "Fold Type" */
			{
			/* TAG BEGIN FFORM_TYPE EVNT:Control */
			extern long profileEventHandler ();
			char option[100];
			WINDOW win;
			OBJECT *data = (OBJECT *) xvt_vobj_get_data (xdWindow);
			...
			if (xvt_list_get_first_sel (xdEvent->v.ctl.ci.win, option, 100))
			{
			   if (strcmp (option, "Sine Fold") == 0)
			   {
			      foldOptions = (FOLD_OPTIONS *) data->options;
			      foldOptions->profile.points = initGraphPoints ((PROFILE_POINT *) foldOptions->profile.points,
			                                                                                               21, SINE_PROFILE, &(foldOptions->profile));
			      foldOptions->profile.joinType = LINE_SEGMENTS;
			      foldOptions->profile.graphLength = 200.0;
			      foldOptions->profile.array = getArrayForProfile (&(foldOptions->profile));
			      initSineFourierCoeff(foldOptions->fourierCoeff);
			   }
			   else
			   {
			      data->generalData = 0;
			...
			/* TAG END FFORM_TYPE EVNT:Control */
			}
			break;
		case FFORM_SINGLE: /* "Single Fold" */
			{
			/* TAG BEGIN FFORM_SINGLE EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, FFORM_SINGLE);
			xvt_ctl_set_checked(ctl_win, (BOOLEAN) !xvt_ctl_is_checked (ctl_win));
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END FFORM_SINGLE EVNT:Control */
			}
			break;
		default:
			break;
