...
/*
	Handler for window ALT_WINDOW ("Form (Alteration)")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
ALT_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
ALT_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CREATE:
		...
		*/
		...
		WINDOW win;
		win = xvt_win_get_ctl (xdWindow, ALT_ALTERATION);
		xvt_list_suspend (win);
		xvt_list_add (win , 0, "None");
		xvt_list_add (win , 1, "Top");
		xvt_list_add (win , 2, "Bottom");
		xvt_list_add (win , 3, "Both");
		xvt_list_resume (win);
		/* TAG END EVNT:Create */
		}
		break;
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case ALT_ALTERATION: /* "Alteration" */
			{
			/* TAG BEGIN ALT_ALTERATION EVNT:Control */
			if (xvt_list_get_sel_index (xdEvent->v.ctl.ci.win) == 0)
			   xvt_vobj_set_enabled (xvt_win_get_ctl(xdWindow, ALT_ALTERATION_EDIT), FALSE);
			else
			   xvt_vobj_set_enabled (xvt_win_get_ctl(xdWindow, ALT_ALTERATION_EDIT), TRUE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END ALT_ALTERATION EVNT:Control */
			}
			break;
		case ALT_ALTERATION_EDIT: /* "Edit..." */
			{
