...
*/
#define WIN_RES_ID VARIABLE_CUBE_WINDOW
...
/*
	Handler for window VARIABLE_CUBE_WINDOW ("Variable Cube size")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
VARIABLE_CUBE_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
VARIABLE_CUBE_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case VARIABLE_CUBE_RATIO: /* "cbox" */
			{
			/* TAG BEGIN VARIABLE_CUBE_RATIO EVNT:Control */
			WINDOW cwin = xvt_win_get_ctl (xdWindow, VARIABLE_CUBE_RATIO);
			if (xvt_list_get_sel_index (cwin) == 0)  /* turned off clever boxing */
			{
				xvt_vobj_set_enabled (cwin, FALSE);
				xvt_vobj_set_title (xvt_win_get_ctl (xdWindow, VARIABLE_CUBE_CUTOFF), "All");
			}
			update3dPreview (NULL_WIN, (OBJECT *) NULL);
			/* TAG END VARIABLE_CUBE_RATIO EVNT:Control */
			}
			break;
		case VARIABLE_CUBE_CUTOFF: /* "nbox" */
			{
			/* TAG BEGIN VARIABLE_CUBE_CUTOFF EVNT:Control */
			/*
			...
			*/
			...
			/* TAG END VARIABLE_CUBE_CUTOFF EVNT:Control */
			}
			break;
		default:
			break;
