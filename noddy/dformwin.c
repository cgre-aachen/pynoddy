...
*/
#define WIN_RES_ID FDYKE_WINDOW
...
/*
	Handler for window FDYKE_WINDOW ("Form (Dyke)")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
FDYKE_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
FDYKE_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case FDYKE_MERGE: /* "Merge Igneous Events" */
			{
			/* TAG BEGIN FDYKE_MERGE EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, FDYKE_MERGE);
			xvt_ctl_set_checked(ctl_win, (BOOLEAN) !xvt_ctl_is_checked (ctl_win));
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END FDYKE_MERGE EVNT:Control */
			}
			break;
		case FDYKE_TYPE: /* "Dyke Type" */
			{
			/* TAG BEGIN FDYKE_TYPE EVNT:Control */
			updateDykeOptions (xdWindow);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END FDYKE_TYPE EVNT:Control */
			}
			break;
		case FDYKE_ALTERATION: /* "alteration" */
			{
			/* TAG BEGIN FDYKE_ALTERATION EVNT:Control */
			updateDykeOptions (xdWindow);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END FDYKE_ALTERATION EVNT:Control */
			}
			break;
		case FDYKE_ALTERATION_EDIT: /* "Edit..." */
			{
			...
			break;
		case FDYKE_PROP_BUTTON: /* "Edit Properties..." */
			{
