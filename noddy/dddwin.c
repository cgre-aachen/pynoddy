...
/*
	Handler for window DDD_WINDOW ("Orientation (Dip, DDir)")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
DDD_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
DDD_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CREATE:
		...
		*/
		...
		setScrollRange (xdWindow, DDD_DDIR_BAR, 0, 360);
		setScrollRange (xdWindow, DDD_DIP_BAR, 0, 180);
		/* TAG END EVNT:Create */
		}
		break;
	case E_DESTROY:
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case DDD_DDIR_BAR: /* "Horizontal Scrollbar Dip Dir" */
			{
			/* TAG BEGIN DDD_DDIR_BAR EVNT:Control */
			int pos = 0;
			/*
			...
			*/
			...
			updateNumericTextField (xdWindow, DDD_DDIR, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END DDD_DDIR_BAR EVNT:Control */
			}
			break;
		case DDD_DIP_BAR: /* "Horizontal Scrollbar Dip" */
			{
			/* TAG BEGIN DDD_DIP_BAR EVNT:Control */
			int pos = 0;
			/*
				Horizontal scrollbar control was operated
			*/
			...
			pos = updateScrollField (xdWindow, xdControlId, pos);
			updateNumericTextField (xdWindow, DDD_DIP, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END DDD_DIP_BAR EVNT:Control */
			}
			break;
		case DDD_DDIR: /* "90" */
			{
			/* TAG BEGIN DDD_DDIR EVNT:Control */
			/*
			...
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				*/
			updateScrollAndText (xdWindow, DDD_DDIR_BAR, DDD_DDIR, TRUE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END DDD_DDIR EVNT:Control */
			}
			break;
		case DDD_DIP: /* "45" */
			{
			/* TAG BEGIN DDD_DIP EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				/*
					Contents of control were changed
				*/
			updateScrollAndText (xdWindow, DDD_DIP_BAR, DDD_DIP, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END DDD_DIP EVNT:Control */
			}
			break;
		default:
			break;
