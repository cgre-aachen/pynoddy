...
*/
#define WIN_RES_ID CORNER_WINDOW
...
/*
	Handler for window CORNER_WINDOW ("Corner")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
CORNER_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
CORNER_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case CORNER_CENTER: /* "Center" */
			{
			/* TAG BEGIN CORNER_CENTER EVNT:Control */
			xdCheckRadioButton(xdWindow, CORNER_CENTER,
				CORNER_CENTER, CORNER_SWUPPER);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END CORNER_CENTER EVNT:Control */
			}
			break;
		case CORNER_SWUPPER: /* "SWUpper Corner" */
			{
			/* TAG BEGIN CORNER_SWUPPER EVNT:Control */
			xdCheckRadioButton(xdWindow, CORNER_SWUPPER,
				CORNER_CENTER, CORNER_SWUPPER);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END CORNER_SWUPPER EVNT:Control */
			}
			break;
		case CORNER_IMPORT_FILE: /* "Select File to Import..." */
			{
			/* TAG BEGIN CORNER_IMPORT_FILE EVNT:Control */
			...
			if (getImportFile (&(options->importFile)))
			{
			   if (loadImportFile (options))
			         loadImportOptions (object);
			}
			
			updateImportOptions (xdWindow, object);
			/* TAG END CORNER_IMPORT_FILE EVNT:Control */
			}
			break;
		default:
			break;
