...
*/
#define WIN_RES_ID LINE_MAP_WINDOW
#define WIN_FLAGS 0x9fL
#define WIN_CLASS ""
...
/*
	Handler for window LINE_MAP_WINDOW ("Line Map")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
LINE_MAP_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
LINE_MAP_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CREATE:
		...
		*/
		...
		RCT bound;
		xvt_vobj_get_client_rect (xdWindow, &bound);
		xvt_sbar_set_range (xdWindow, HSCROLL, 0, bound.right-bound.left);
		xvt_sbar_set_range (xdWindow, VSCROLL, 0, bound.bottom-bound.top);
		xvt_sbar_set_pos (xdWindow, HSCROLL, 0);
		xvt_sbar_set_pos (xdWindow, VSCROLL, 0);
		/* TAG END EVNT:Create */
		}
		break;
	case E_DESTROY:
	...
	case E_HSCROLL:
		...
		*/
		...
		case SC_LINE_UP:
			pos = xvt_sbar_get_pos (xdWindow, HSCROLL) - 1;
			break;
		case SC_LINE_DOWN:
			pos = xvt_sbar_get_pos (xdWindow, HSCROLL) + 1;
			break;
		case SC_PAGE_UP:
			pos = xvt_sbar_get_pos (xdWindow, HSCROLL) - 20;
			break;
		case SC_PAGE_DOWN:
			pos = xvt_sbar_get_pos (xdWindow, HSCROLL) + 20;
			break;
		...
		oldPos = xvt_sbar_get_pos (xdWindow, HSCROLL);
		xvt_sbar_set_pos (xdWindow, HSCROLL, pos);
		xvt_vobj_get_client_rect (xdWindow, &bound);
		xvt_dwin_scroll_rect (xdWindow, &bound, 0, oldPos - pos);
	...
	case E_VSCROLL:
		...
		*/
		switch (xdEvent->v.scroll.what) {
		case SC_LINE_UP:
			pos = xvt_sbar_get_pos (xdWindow, VSCROLL) - 1;
			break;
		case SC_LINE_DOWN:
			pos = xvt_sbar_get_pos (xdWindow, VSCROLL) + 1;
			break;
		case SC_PAGE_UP:
			pos = xvt_sbar_get_pos (xdWindow, VSCROLL) - 20;
			break;
		case SC_PAGE_DOWN:
			pos = xvt_sbar_get_pos (xdWindow, VSCROLL) + 20;
			break;
		...
		oldPos = xvt_sbar_get_pos (xdWindow, VSCROLL);
		xvt_sbar_set_pos (xdWindow, VSCROLL, pos);
		xvt_vobj_get_client_rect (xdWindow, &bound);
		xvt_dwin_scroll_rect (xdWindow, &bound, 0, oldPos - pos);
