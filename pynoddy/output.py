...
class NoddyOutput(object):
    ...
    def plot_section(self, direction='y', position='center', **kwds):
        ...
        **Optional Keywords**:
            ...
            - *colorbar* = bool : plot colorbar (default: False)
            - *colorbar_orientation* = 'horizontal' or 'vertical' : orientation of colorbar
                    (default: 'vertical')
            - *colorbar_label* = string : label for colorbar (default: "Lihtology")
            - *colorbar_type* = 'discrete', 'continuous': type of colorbar (default: 'discrete')
        ...
        colorbar = kwds.get("colorbar", False)
        colorbar_label = kwds.get("colorbar_label", "Lithology")
        colorbar_type = kwds.get("colorbar_type", "discrete")
        ...
        if colorbar and not kwds.has_key('ax'):  # disable - color bar is broken
            ...
            if colorbar_type == 'discrete':
                cmap = plt.cm.jet
                bounds = np.arange(np.min(section_slice), np.max(section_slice)+2)
                norm = mpl.colors.BoundaryNorm(bounds, cmap.N)

            else:
                # No boundary normalisation
                cmap = plt.cm.jet
                norm = mpl.colors.Normalize(np.min(section_slice), np.max(section_slice))
                # norm = mpl.colors.BoundaryNorm(bounds, cmap.N)
            ...
            if cbar_orientation == 'horizontal':
                if colorbar_type == 'discrete':
                    ...
                    cb = mpl.colorbar.ColorbarBase(ax2, cmap=cmap_type, norm=norm, spacing='proportional',
                                                   ticks=bounds, boundaries=bounds - 0.5, label=colorbar_label,
                                                   orientation='horizontal')  # , format='%s')
                else:  # continuous values
                    ax2 = fig.add_axes([0.125, 0.18, 0.775, 0.04])
                    cb = mpl.colorbar.ColorbarBase(ax2, cmap=cmap_type, norm=norm, # spacing='proportional',
                                                   # ticks=bounds, # boundaries=bounds - 0.5,
                ...
                if (S.has_edge(e[0], e[1])):  # edge already exists
                    ...
                    try:
                        if not S.has_node(e[0]):  # add node
                            ...
                            S.node[e[0]]['centroid'] = (
                            np.mean([c1[0], c2[0]]), np.mean([c1[1], c2[1]]), np.mean([c1[2], c2[2]]),)

                            S.node[e[0]]['volume'] = np.mean(
                                [float(G.node[e[0]]['volume']), float(S.node[e[0]]['volume'])])
                        ...
                        if not S.has_node(e[1]):  # add node
                            ...
                            S.node[e[1]]['centroid'] = (
                            np.mean([c1[0], c2[0]]), np.mean([c1[1], c2[1]]), np.mean([c1[2], c2[2]]),)
                            S.node[e[1]]['volume'] = np.mean(
                                [float(G.node[e[1]]['volume']), float(S.node[e[1]]['volume'])])
