...
*/
#define WIN_RES_ID RANGE_WINDOW
...
/*
	Handler for window RANGE_WINDOW ("calculation range")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
RANGE_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
RANGE_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case RANGE_CONV: /* "range" */
			{
			/* TAG BEGIN RANGE_CONV EVNT:Control */
#include <math.h>
			/*
			...
			*/
			...
			/* TAG END RANGE_CONV EVNT:Control */
			}
			break;
		case RANGE_ANALYTIC: /* "Analytic" */
			{
			/* TAG BEGIN RANGE_ANALYTIC EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
			...
			/* TAG END RANGE_ANALYTIC EVNT:Control */
			}
			break;
		case RANGE_ALL_EXACT: /* "All" */
			{
			/* TAG BEGIN RANGE_ALL_EXACT EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, RANGE_ALL_EXACT);
			xvt_ctl_set_checked(ctl_win, !xvt_ctl_is_checked(ctl_win));
			updateOptions (xdWindow, GEOPHYSICS_CALC_OP);
			update3dPreview (NULL_WIN, (OBJECT *) NULL);
			/* TAG END RANGE_ALL_EXACT EVNT:Control */
			}
			break;
		case RANGE_REAL_GEOLOGY_PADDING: /* "Generate Extra Geology for Padding" */
			{
			/* TAG BEGIN RANGE_REAL_GEOLOGY_PADDING EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, RANGE_REAL_GEOLOGY_PADDING);
			xvt_ctl_set_checked(ctl_win, !xvt_ctl_is_checked(ctl_win));
			/* TAG END RANGE_REAL_GEOLOGY_PADDING EVNT:Control */
			}
			break;
		default:
			break;
