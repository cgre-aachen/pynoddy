...
*/
#define WIN_RES_ID NEWWIN_WINDOW
...
/*
	Handler for window NEWWIN_WINDOW ("new win each stage")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
NEWWIN_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
NEWWIN_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CONTROL:
		...
		*/
		...
		switch(xdControlId) {
		case NEWWIN_EACHTIME: /* "New window each time" */
			{
			/* TAG BEGIN NEWWIN_EACHTIME EVNT:Control */
			WINDOW ctl_win = xvt_win_get_ctl(xdWindow, NEWWIN_EACHTIME);
			xvt_ctl_set_checked(ctl_win, (BOOLEAN) (!xvt_ctl_is_checked(ctl_win)));
			/* TAG END NEWWIN_EACHTIME EVNT:Control */
			}
			break;
		default:
			break;
