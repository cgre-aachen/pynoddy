...
/*
	Handler for window XYZ_WINDOW ("Position")
*/
long XVT_CALLCONV1
#if XVT_CC_PROTO
XYZ_WINDOW_eh XVT_CALLCONV2 (WINDOW xdWindow, EVENT *xdEvent)
#else
XYZ_WINDOW_eh XVT_CALLCONV2 (xdWindow, xdEvent)
WINDOW xdWindow;
EVENT *xdEvent;
#endif
{
	...
	case E_CREATE:
		...
		*/
		...
		setScrollRange (xdWindow, XYZ_X_BAR, -10000, 10000);
		setScrollRange (xdWindow, XYZ_Y_BAR, -10000, 10000);
		setScrollRange (xdWindow, XYZ_Z_BAR, -10000, 10000);
		/* TAG END EVNT:Create */
		}
		break;
	case E_DESTROY:
		...
		case XYZ_X: /* "x" */
			...
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				*/
			updateScrollAndText (xdWindow, XYZ_X_BAR, XYZ_X, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END XYZ_X EVNT:Control */
			}
			break;
		case XYZ_Z: /* "z" */
			{
			/* TAG BEGIN XYZ_Z EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				/*
					Contents of control were changed
				*/
			updateScrollAndText (xdWindow, XYZ_Z_BAR, XYZ_Z, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END XYZ_Z EVNT:Control */
			}
			break;
		case XYZ_Y: /* "y" */
			{
			/* TAG BEGIN XYZ_Y EVNT:Control */
			/*
				Edit control was operated.
			*/
			if (xdEvent->v.ctl.ci.v.edit.focus_change) {
				...
				/*
					Contents of control were changed
				*/
			updateScrollAndText (xdWindow, XYZ_Y_BAR, XYZ_Y, FALSE);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			}
			/* TAG END XYZ_Y EVNT:Control */
			}
			break;
		case XYZ_X_BAR: /* "Position X Bar" */
			{
			/* TAG BEGIN XYZ_X_BAR EVNT:Control */
			int pos = 0;
			/*
			...
			*/
			...
			updateNumericTextField (xdWindow, XYZ_X, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END XYZ_X_BAR EVNT:Control */
			}
			break;
		case XYZ_Y_BAR: /* "Position Y Bar" */
			{
			/* TAG BEGIN XYZ_Y_BAR EVNT:Control */
			int pos = 0;
			/*
				Horizontal scrollbar control was operated
			*/
			...
			pos = updateScrollField (xdWindow, xdControlId, pos);
			updateNumericTextField (xdWindow, XYZ_Y, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END XYZ_Y_BAR EVNT:Control */
			}
			break;
		case XYZ_Z_BAR: /* "Position Z Bar" */
			{
			/* TAG BEGIN XYZ_Z_BAR EVNT:Control */
			int pos = 0;
			/*
				Horizontal scrollbar control was operated
			*/
			...
			pos = updateScrollField (xdWindow, xdControlId, pos);
			updateNumericTextField (xdWindow, XYZ_Z, pos);
			update3dPreview (xdWindow, (OBJECT *) xvt_vobj_get_data (xdWindow));
			/* TAG END XYZ_Z_BAR EVNT:Control */
			}
			break;
		default:
			break;
